...
for i, sector in enumerate(sectorTable['sector']):
    ...
    for j, designation in enumerate(gaia_targets['designation']):
        pixel = wcs_projection[f'wcs_{sector}'].all_world2pix(
...
bar = ChargingBar('Sampling Best Trial Periods: ', max=100,
                  suffix='%(percent).1f%% Elapsed: %(elapsed)ds Remaining: %(eta)ds')
...
np.savetxt(location + 'modified_periods.csv', mod_periods.reshape((first_trial, second_trial)).transpose(), fmt='%.6e',
           delimiter=",")

plt.figure(figsize=(15, 8))
plt.plot(sample_period, std, lw=.2, c='silver')
plt.plot(sample_period[np.where(std < float(stdv_threshold))], std[np.where(std < float(stdv_threshold))], '.', c='C3',
         ms=.5)
...
def cnn_prediction(coord):
    ...
    for sector in sectorTable['sector'][1:]:
        ...
        if new_i >= 0 and new_i < size and new_l >= 0 and new_l < size and np.mean(
                bkgSubFlux['flux_' + str(sector)][:, new_i, new_l]) > 0.1:
    ...
    if min(flux_raw) < 0:
        ...
        flux_1d = flatten(time_raw, flux_raw, break_tolerance=0.1, window_length=0.5, edge_cutoff=0.25,
                          return_trend=False)
        ...
        if np.max(predict) < 0.5:
            ...
            if np.max(predict) >= float(quality):
                ...
                ax4.errorbar(t_pf, flux_1d * mean, flux_err_1d, marker='.', ms=2, ls='', elinewidth=0.4,
                             ecolor='silver', c='darkgrey', zorder=1)
                ...
                data = Table([time_1d, flux_1d * mean, flux_err_1d, quality_1d],
                             names=['TBJD', 'bkgsubflux', 'flux_err', 'quality'])
                ascii.write(data, location + 'TESS_' + str(target_name) + '[' + str(x) + ',' + str(y) + '].dat',
                            overwrite=True)
            else:
                pass
